...
class KafkaService:
    def __init__(self):
        self.bootstrap_servers = os.getenv('KAFKA_BOOTSTRAP_SERVERS', 'kafka:9093')
        self.security_protocol = os.getenv('KAFKA_SECURITY_PROTOCOL', 'SASL_PLAINTEXT')
        self.sasl_mechanism = os.getenv('KAFKA_SASL_MECHANISM', 'PLAIN')
        self.sasl_username = os.getenv('KAFKA_SASL_USERNAME', 'product_service')
        self.sasl_password = os.getenv('KAFKA_SASL_PASSWORD', 'product-secret')
...
async def send_product_registration_message(product_id: int):
    ...
    try:
        await KafkaService.producer.send_and_wait(
